  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Nouakchott Négoce SARL"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Nouakchott Négoce SARL" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest avenant-ceiling-violated-is-held-and-unoverridable
  (testing "declared avenant (25% of base marché amount) exceeds Code des Marchés Publics Art.53's 20% ceiling -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:avenant-ceiling-violated} (-> (store/ledger db) last :basis)))
...
(deftest nif-record-unverified-is-held-and-unoverridable
  (testing "unverified DGI NIF registration when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:nif-record-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))

(deftest avenant-within-ceiling-is-not-gated-by-the-flagship-check
  (testing "eng-6 declares a cumulative avenant that IS within Art.53's 20% ceiling (15%) -- the flagship check never fires for it"
    (let [[_db actor] (fresh)
