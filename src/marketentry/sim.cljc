...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (MRT, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up avenant-ceiling-violated) ==")
    ...
    (println "== filing/submit eng-4 (declared avenant is 25% of base marché, outside Art.53's 20% ceiling -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up NIF-unregistered) ==")
    ...
    (println "== filing/submit eng-5 (NIF registration unverified -> HARD hold) ==")
    (println (exec-op actor "t11" {:op :filing/submit :subject "eng-5"} operator))

    (println "== jurisdiction/assess eng-6 (sets up an avenant WITHIN the 20% ceiling -- clean) ==")
    ...
    (println "== filing/submit eng-6 (declared avenant is 15% of base marché, WITHIN Art.53's 20% ceiling -- clean submit, still escalates for approval) ==")
