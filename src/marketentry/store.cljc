(ns marketentry.store
  "SSoT for the Islamic Republic of Mauritania (MRT) market-entry
  ...
  Demo `:operator` company names below are FICTIONAL (Nouakchott/
  Nouadhibou-flavored placeholder names for the offline demo/test
  suite), not real Mauritanian entities."
...
(defn demo-data
  ...
  `:prior-avenant?` / `:avenant-cumulative-amount` / `:marche-base-
  amount` are ground truth for the flagship SINGLE-SIDED
  PERCENTAGE-OF-BASE-VALUE CEILING check (Code des Marchés Publics
  Art. 53 -- does a declared cumulative avenant value exceed the 20%
  statutory ceiling of the declared base marché amount?);
  `:requires-nif?` / `:nif-verified?` are ground truth for the
  conditional Direction Générale des Impôts (DGI) NIF check."
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Nouakchott Négoce SARL" :procurement-channel "ARMP (marchespublics.gov.mr) tender notice"
             ...
             :jurisdiction "MRT" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :procurement-channel "ARMP (marchespublics.gov.mr) tender notice"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :prior-avenant? false
             :requires-nif? true :nif-verified? true
    ...
    "eng-3" {:id "eng-3" :operator "Nouadhibou Pêche et Logistique SA" :procurement-channel "ARMP (marchespublics.gov.mr) tender notice"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "MRT" :status :intake}
    "eng-4" {:id "eng-4" :operator "Trarza Travaux Publics SARL" :procurement-channel "ARMP (marchespublics.gov.mr) tender notice"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :prior-avenant? true :avenant-cumulative-amount 250000 :marche-base-amount 1000000
             :requires-nif? true :nif-verified? true
             :drafted? false :submitted? false
             :jurisdiction "MRT" :status :intake}
    "eng-5" {:id "eng-5" :operator "Adrar Import-Export SARL" :procurement-channel "ARMP (marchespublics.gov.mr) tender notice"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :prior-avenant? false
             ...
             :drafted? false :submitted? false
             :jurisdiction "MRT" :status :intake}
    "eng-6" {:id "eng-6" :operator "Tiris Zemmour Construction et Cie" :procurement-channel "ARMP (marchespublics.gov.mr) tender notice"
             :base-fee 400000 :monthly-rate 25000 :monitoring-months 6
             :claimed-fee 550000.0
             :prior-avenant? true :avenant-cumulative-amount 150000 :marche-base-amount 1000000
             :requires-nif? true :nif-verified? true
             :drafted? false :submitted? false
             :jurisdiction "MRT" :status :intake}}})
...
(defn- engagement->tx [{:keys [id operator procurement-channel base-fee monthly-rate monitoring-months claimed-fee
                               prior-avenant? avenant-cumulative-amount marche-base-amount
  ...
  (cond-> {:engagement/id id}
    ...
    (some? prior-avenant?)                            (assoc :engagement/prior-avenant? prior-avenant?)
    avenant-cumulative-amount                         (assoc :engagement/avenant-cumulative-amount avenant-cumulative-amount)
    marche-base-amount                                (assoc :engagement/marche-base-amount marche-base-amount)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/procurement-channel :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/prior-avenant? :engagement/avenant-cumulative-amount :engagement/marche-base-amount
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :procurement-channel (:engagement/procurement-channel m)
     ...
     :prior-avenant? (boolean (:engagement/prior-avenant? m))
     :avenant-cumulative-amount (:engagement/avenant-cumulative-amount m)
     :marche-base-amount (:engagement/marche-base-amount m)
