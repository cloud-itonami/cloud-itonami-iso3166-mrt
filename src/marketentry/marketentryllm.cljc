(ns marketentry.marketentryllm
  ...
  the Islamic Republic of Mauritania (MRT) public-sector market-entry
  compliance actor.
  ...
  cited), never a committed record or a real tender response / ARMP
...
(defn- propose-submit
  ...
  :actuation/submit-filing` -- real-world tender response / ARMP
  filing submission."
  [db {:keys [subject]}]
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向け提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "nif-verified?=" (:nif-verified? e)
                        " prior-avenant?=" (:prior-avenant? e)
                        " avenant-cumulative-amount=" (:avenant-cumulative-amount e)
                        " marche-base-amount=" (:marche-base-amount e)
